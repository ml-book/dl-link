        ...
        while True:
            try:
                async with websockets.connect(ip) as websocket:
                    print(f"{websocket.local_address} Connected to {ip}")
...
class CSManager:
    ...
    async def forward(self, packet: RecvRemotePacket):
        ...
        if addr in self.functions: # 如果目标就在本机
            ...
            elif self.client.has_link(addr): # 这一项貌似永远为非，因为后端没有client的数据
                # client发 server接
...
class Send_to_client:
    ...
    def __init__(self) -> None:
        cs: CSManager = CSManager.get_instance()
        self.sender = cs.register(self.run, f'{cs.server.addr_prefix}/send-to-client')

    async def run(self, config):
        cs: CSManager = CSManager.get_instance()
        await self.sender.send(config, f'{cs.client.addr_prefix}/test-local')
...
if __name__ == '__main__':
    ...
    if whoami == 'server':
        ...
        # async def main():
        #     await send_to_client.run("from server to client")
        # asyncio.run(main())
